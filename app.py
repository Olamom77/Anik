...
def init_db():
    ...
    c.execute("INSERT OR IGNORE INTO admins (username,password,name) VALUES (?,?,?)",
              ('admin', hash_pw('admin123'), 'Site Administrator'))
    # Seed student
...
def email_contact_confirm(name, to):
    send_email(to, 'We received your message – Anik Tech School', f"""
    <div style="font-family:sans-serif;max-width:560px;margin:0 auto;padding:32px;">
      <h2 style="color:#185FA5;font-family:Georgia,serif;">Anik Tech School</h2>
      <p>Dear {name},</p>
      <p>Thank you for contacting us! We have received your message and will get back to you within <strong>24 hours</strong>.</p>
      <p style="color:#555;">For urgent matters, call us: <strong>+234 (0) 800 ANIK TECH</strong></p>
      <hr style="border:none;border-top:1px solid #eee;margin:24px 0;">
      <p style="font-size:12px;color:#999;">© 2026 Anik Tech School · 1 Anik Tech Avenue, Jabi District, Abuja, Nigeria.</p>
    </div>""")

def email_welcome(name, to, sid, pw):
    send_email(to, f'Your Anik Tech Student Account – {sid}', f"""
    <div style="font-family:sans-serif;max-width:560px;margin:0 auto;padding:32px;">
      <h2 style="color:#185FA5;font-family:Georgia,serif;">Welcome to Anik Tech School!</h2>
      <p>Dear {name},</p>
      ...
      <div style="background:#f5f5f5;padding:16px;border-radius:8px;margin:16px 0;">
        ...
        <p><strong>Portal:</strong> <a href="http://localhost:5000/student/login" style="color:#185FA5;">student portal</a></p>
      </div>
      <p style="color:#c00;font-size:13px;">⚠ Please change your password after first login.</p>
      <hr style="border:none;border-top:1px solid #eee;margin:24px 0;">
      <p style="font-size:12px;color:#999;">© 2026 Anik Tech School.</p>
    </div>""")
...
def contact():
    if request.method == 'POST':
        ...
        if name and email and message:
            ...
            db.commit(); db.close()
            email_contact_confirm(name, email)
...
def admin_add_student():
    if request.method == 'POST':
        ...
        try:
            ...
            db.commit()
            email_welcome(name, email, sid, pw)
            flash(f'Student {name} ({sid}) added. Welcome email sent. ✓', 'success')
        except Exception as e:
...
def student_grades():
    ...
    stu = db.execute("SELECT * FROM students WHERE student_id=?", (session['student_id'],)).fetchone()
    grd = db.execute("""
        SELECT g.score,g.grade,g.semester,c.code,c.title,c.units
        FROM grades g JOIN courses c ON g.course_id=c.id
        WHERE g.student_id=? ORDER BY g.semester,c.code
